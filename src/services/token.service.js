...
import { user_msg } from '../constants/res.message.js'
...
export const sendToken = (req, res, user, statusCode, message, deletionToken = null) => {
    try {
        ...
        const cookieOptions = {
            ...
            expires: new Date(Date.now() + Number(config.JWT_COOKIE_EXPIRE) * 24 * 60 * 60 * 1000) //* 24 * 60 * 60 * 1000
        }
        ...
        logger.info(user_msg.token_created(userData.name))
    } catch (error) {
        ...
        httpError('TOKEN ERROR', handleError, error, req, 500)
    }
}
